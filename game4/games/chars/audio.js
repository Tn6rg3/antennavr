// game4/games/chars/audio.js
// Gestione Audio Indipendente per il modulo Chars
        ...
        for (let char of text.toUpperCase()) {
            ...
            if (MORSE_DICT[char]) {
                for (let i = 0; i < MORSE_DICT[char].length; i++) {
                    ...
                    time += duration;
                    if (i < MORSE_DICT[char].length - 1) time += charUnit;
                }
                time += (3 * charUnit); // Spazio tra caratteri
            } else if (char === ' ') {
                time += (7 * charUnit); // Spazio tra parole
            }
        }
        setTimeout(() => {
