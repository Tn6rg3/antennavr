    ...
    if (modeValue === 'room') {
        if (roomCode) {
            db.ref(`rooms/${roomCode}/players`).once('value', snap => {
                window.renderRoomLeaderboard(snap.val() || {});
            });
        } else if (window.lastFinishedRoomCode) {
            db.ref(`rooms/${window.lastFinishedRoomCode}/players`).once('value', snap => {
                window.renderRoomLeaderboard(snap.val() || {});
            });
        } else {
            els.leaderboardContainer.innerHTML = `<p style="text-align:center; padding:20px; color:var(--hint-color);">${currentLang==='it'?'Nessuna sessione attiva trovata.':'No active session found.'}</p>`;
        }
