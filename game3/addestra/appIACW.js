    ...
    try {
        if (typeof firebase !== 'undefined' && firebase.database) {
            ...
            if (foundUrls.length > 0) {
                ...
                console.log("🔒 Loaded fresh Apps Script URLs dynamically from Firebase Database:", window.aiAllFirebaseUrls);
                return window.aiActiveAddestraUrl;
            }
        }
    } catch(e) {
...
async function loadSelectedQSO() {
    ...
    // Fallback Player Iframe Google Drive (Identico a Ascolta QSO Reali)
    if (fileId && iframeEl) {
        iframeEl.src = `https://drive.google.com/file/d/${fileId}/preview`;
        iframeEl.style.display = 'block';
        if (statusElem) {
            statusElem.innerText = "🎧 Player Google Drive Attivato (Ascolta dal Player qui sopra)";
