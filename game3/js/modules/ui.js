...
function showScreen(screenId) {
    clearAllTimers(); if (document.activeElement?.blur) document.activeElement.blur();
    document.querySelectorAll('.screen').forEach(el => el.classList.remove('active-screen'));
    if (els[screenId]) els[screenId].classList.add('active-screen');
    hideChat(); if (els.matchDetailsModal) els.matchDetailsModal.style.display = 'none';
    ...
    if (screenId === 'setupScreen') {
        ...
        else if (lR && els.rejoinContainer) { els.rejoinContainer.style.display = 'block'; if (els.rejoinGameBtn) els.rejoinGameBtn.onclick = () => { roomCode = lR; isRejoining = true; joinRoomLogic(false); }; }
        listenToOnlineUsers(); listenToRooms();
    } else {
    ...
    if (isPlayingS) {
        ...
        if (screenId === 'lobbyScreen' || screenId === 'gameArea') { activeChatContext = 'room'; listenToChat(); } else activeChatContext = null;
    } else if (screenId === 'teamsScreen') { activeChatContext = 'team'; checkMyTeamStatus(); }
    else { if (screenId === 'participationScreen') switchActTab('daily'); if (activeChatContext !== 'global') { activeChatContext = 'global'; listenToChat(); } }
}
...
function showLeaderboardTab(tabId) {
    ...
    if (modeV === 'room') {
        ...
        populateDynamicFilters('pingpong', ''); fetchAndRenderGlobalLeaderboard('pingpong', els.lbWordFilter?.value || 'all');
    } else {
        if (els.lbFilterArea) els.lbFilterArea.style.display = 'block'; if (els.roomWinnerBanner) els.roomWinnerBanner.style.display = 'none'; if (els.waitingOthersText) els.waitingOthersText.style.display = 'none';
        ...
        let fP = isM ? `recent_matches/${baseM}_multi` : baseM; populateDynamicFilters(fP, isM ? '' : 'single');
        fetchAndRenderGlobalLeaderboard(`${baseM}_${type}`, els.lbWordFilter?.value || 'all');
    }
}
...
function renderRoomLeaderboard(players) {
    ...
    if (allF && (roomCode && (roomCode.startsWith("TRN_") || currentMode === 'pingpong' || pA.length > 1))) renderHeadToHeadView(pA, els.leaderboardContainer);
    else {
        pA.sort((a, b) => (b.score - a.score) || (b.wpm - a.wpm)).forEach((player, idx) => {
            ...
            if (player.username) { const nL = document.createElement('span'); nL.style.cssText = "color:var(--link-color); cursor:pointer; text-decoration:underline;"; nL.textContent = player.name; nL.onclick = () => openTelegramProfile(player.username); lS.appendChild(nL); }
...
function renderHeadToHeadView(players, container) {
    ...
    players.forEach((p) => {
        ...
        card.onclick = () => { if (p.id !== myId) return showToast(currentLang === 'it' ? "Puoi vedere solo i tuoi dettagli." : "You can only view your own details."); if (p.matchDetails?.length > 0) showPlayerDetailsModal(p.name, p.matchDetails); else if (p.id === myId && matchDetailsArray.length > 0) showPlayerDetailsModal(p.name, matchDetailsArray); else showToast(currentLang === 'it' ? "Dettagli non disponibili" : "Details not available"); };
...
function fetchAndRenderGlobalLeaderboard(tabType, filterWordCount) {
    ...
    if (tabType === 'daily_challenge') { let tS = new Date().toISOString().split('T')[0]; db.ref(`leaderboard/daily_challenge/${tS}`).orderByChild('score').limitToLast(50).once('value', snapshot => { let p = []; if (snapshot.exists()) snapshot.forEach(c => { if (c.val()) p.push(c.val()); }); p.sort((a, b) => (Number(b.score) || 0) - (Number(a.score) || 0)); renderPlayersListHTML(p.slice(0, 50), els.leaderboardContainer, false); }); return; }
    if (['standard_multi', 'chars_multi', 'quiz_multi'].includes(tabType)) { db.ref(`leaderboard/recent_matches/${tabType}`).once('value', snapshot => { let m = []; snapshot.forEach(wcN => { if (filterWordCount === 'all' || wcN.key === filterWordCount) wcN.forEach(mN => m.push(mN.val())); }); m.sort((a,b) => (b.ts || 0) - (a.ts || 0)); renderMatchesHistoryHTML(m.slice(0, 20), els.leaderboardContainer); }); return; }
    if (tabType === 'pingpong') {
        if (filterWordCount !== 'all') { db.ref(`leaderboard/pingpong/${filterWordCount}`).orderByChild('score').limitToLast(50).once('value', snapshot => { let p = []; if (snapshot.exists()) snapshot.forEach(uN => { if (uN.val()) p.push(uN.val()); }); p.sort((a, b) => (Number(b.score) || 0) - (Number(a.score) || 0)); renderPlayersListHTML(p.slice(0, 50), els.leaderboardContainer, true); }); }
        else { db.ref(`leaderboard/pingpong`).once('value', snapshot => { let p = []; if (snapshot.exists()) snapshot.forEach(wcN => { wcN.forEach(uN => { if (uN.val()) p.push(uN.val()); }); }); p.sort((a, b) => (Number(b.score) || 0) - (Number(a.score) || 0)); renderPlayersListHTML(p.slice(0, 50), els.leaderboardContainer, true); }); }
        return;
    }
    if (tabType === 'callsign') { db.ref('leaderboard/callsign/global').orderByChild('score').limitToLast(50).once('value', snapshot => { let p = []; if (snapshot.exists()) snapshot.forEach(c => { if (c.val()) p.push(c.val()); }); p.sort((a, b) => (Number(b.score) || 0) - (Number(a.score) || 0)); renderPlayersListHTML(p.slice(0, 50), els.leaderboardContainer, false); }); return; }
    if (tabType === 'tournaments') { db.ref('leaderboard/tournaments').orderByChild('score').limitToLast(50).once('value', snapshot => { let t = []; if (snapshot.exists()) snapshot.forEach(c => { if (c.val()) t.push(c.val()); }); t.sort((a, b) => (Number(b.score) || 0) - (Number(a.score) || 0)); renderPlayersListHTML(t.slice(0, 50), els.leaderboardContainer, false, true); }); return; }
    ...
    if (filterWordCount !== 'all') { db.ref(`leaderboard/${mP}/${sT}_${filterWordCount}`).orderByChild('score').limitToLast(50).once('value', snapshot => { let p = []; if (snapshot.exists()) snapshot.forEach(uN => { if (uN.val()) p.push(uN.val()); }); p.sort((a, b) => (Number(b.score) || 0) - (Number(a.score) || 0)); renderPlayersListHTML(p.slice(0, 50), els.leaderboardContainer, true); }); }
    else { db.ref(`leaderboard/${mP}`).once('value', snapshot => { let p = []; if (snapshot.exists()) snapshot.forEach(wcN => { if (!wcN.key.startsWith(sT + "_")) return; wcN.forEach(uN => { if (uN.val()) p.push(uN.val()); }); }); p.sort((a, b) => (Number(b.score) || 0) - (Number(a.score) || 0)); renderPlayersListHTML(p.slice(0, 50), els.leaderboardContainer, true); }); }
}

function renderMatchesHistoryHTML(matches, container) { container.innerHTML = ''; if (matches.length === 0) { const p = document.createElement('p'); p.style.cssText = 'text-align:center; color:var(--hint-color);'; p.textContent = currentLang === 'it' ? 'Nessuna sfida recente trovata.' : 'No recent challenges found.'; container.appendChild(p); return; } matches.forEach(match => { const mw = document.createElement('div'); mw.style.cssText = "margin-bottom:25px; border-bottom:1px dashed var(--hint-color); padding-bottom:15px;"; const iD = document.createElement('div'); iD.style.cssText = 'text-align:center; font-size:0.8em; color:var(--hint-color); margin-bottom:8px;'; iD.textContent = `📅 ${match.date} - ${match.wordCount} Stringhe`; mw.appendChild(iD); renderHeadToHeadView(match.players, mw); container.appendChild(mw); }); }
...
async function checkActivityAndAwardMedals() {
    ...
    try {
        ...
        const ch = (c, th, id, t, d, i, p) => { if (c >= th && (!myM[id] || myM[id].periodKey !== p)) { awardMedal(id, t, d, i, p); myM[id] = { periodKey: p }; return true; } return false; };
...
window.showProfileScreen = function() {
    showScreen('profileScreen'); if (els.errorChartContainer) els.errorChartContainer.textContent = 'Caricamento...'; if (els.wpmErrorChartContainer) els.wpmErrorChartContainer.textContent = 'Caricamento...'; if (els.matchHistoryList) els.matchHistoryList.textContent = 'Caricamento...';
