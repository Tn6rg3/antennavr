...
function addOrUpdateRoomCard(code, room) {
    ...
    if (code.startsWith("TRN_") || (room.expiresAt && Date.now() > room.expiresAt) || room.status !== 'waiting' || room.type === 'single') { removeRoomCard(code); return; }
    ...
    if (room.hostId === myId && pC > prevC && pC >= 2 && (roomCode !== code || !els.lobbyScreen.classList.contains('active-screen'))) {
        showToast(`👤 Un giocatore è appena entrato nella tua stanza #${code}!`); playNotificationSound();
    }
    ...
    const btn = document.createElement('button'); btn.className = 'action-btn-small'; btn.textContent = currentLang === 'en' ? 'Join' : 'Entra'; btn.onclick = () => window.joinSpecificRoom(code); li.appendChild(btn);
}
...
function listenToRooms() {
    ...
    const onA = lQ.on('child_added', snap => addOrUpdateRoomCard(snap.key, snap.val()));
    const onC = lQ.on('child_changed', snap => addOrUpdateRoomCard(snap.key, snap.val()));
    const onR = lQ.on('child_removed', snap => removeRoomCard(snap.key));
...
function joinRoomLogic(isReconnect = false) {
    ...
    pRef.once('value', snapshot => {
        ...
        if (pD?.finished) { showScreen('leaderboardScreen'); activeTab = "room"; showLeaderboardTab('tabRoomBtn'); localStorage.removeItem(STORAGE_ROOM_KEY); return; }
        ...
        if (!pD) {
            pRef.set({ name: myName, username: myPrivacy ? "" : tgUsername, score: 0, wpm: 0, finished: false, teamId: myTeamId, ready: false, online: true }).then(() => {
                if (!isSinglePlayer && !roomCode.startsWith("TRN_")) {
                    db.ref(`rooms/${roomCode}/players`).once('value', s => { db.ref(`public_lobby_rooms/${roomCode}/pCount`).set(s.exists() ? Object.keys(s.val()).length : 1); });
                }
            });
        ...
        listeners.room.on('value', snap => {
            ...
            if (rD.status === 'playing' && !gameRunning) { currentWpm = rD.wpm; baseWpm = rD.wpm; currentTone = rD.tone; if (rD.words) gameWords = rD.words; return resumeGameSequence(); }
            if (rD.status === 'countdown' && !gameRunning) { currentWpm = rD.wpm; baseWpm = rD.wpm; currentTone = rD.tone; if (rD.words) gameWords = rD.words; return startCountdownSequence(); }
            if (rD.status === 'waiting') {
                renderPlayersList(rD.players || {}, rD.hostId);
                const pC = Object.keys(rD.players || {}).length;
                if (myId === rD.hostId && pC > lastPlayerCount && activeChatContext !== 'room') showRoomEventModal("Qualcuno è entrato!", "Un nuovo giocatore è appena entrato.");
...
function renderPlayersList(playersData, hostId) {
    ...
    Object.entries(playersData).forEach(([id, data]) => {
        ...
        if (data.username && String(data.username).trim() !== "") { nS.style.color = 'var(--link-color)'; nS.style.cursor = 'pointer'; nS.style.textDecoration = 'underline'; nS.onclick = () => openTelegramProfile(data.username); }
...
function exitRoomCleanly(roomWasDeletedByHost = false, isExplicitQuit = false) {
    clearAllTimers(); if (typeof window.currentSpectatorCleanup === 'function') { window.currentSpectatorCleanup(); window.currentSpectatorCleanup = null; }
    ...
    hideChat(); showScreen(targetS); if (targetS === 'setupScreen') listenToRooms();
}
