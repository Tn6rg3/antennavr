...
function listenToRoomLeaderboard() {
    ...
    listeners.roomLb = db.ref(`rooms/${roomCode}`).on('value', snap => {
        ...
        if (allF && roomData.status !== 'finished' && Object.keys(players).length > 0) {
            db.ref(`rooms/${roomCode}/status`).set('finished'); if (Object.keys(players).length >= 2 && ['multi', 'pingpong', 'chars', 'quiz'].includes(roomData.type || currentMode)) saveMatchToGlobalHistory(players, roomData);
            if (roomCode.startsWith("TRN_")) {
                ...
                if (wTI && activeTrnId) { db.ref(`tournaments/${activeTrnId}/matches/${mId}`).update({ status: 'finished', winnerTeamId: wTI }).then(() => checkTournamentCompletion(activeTrnId)); if (wTI !== "tie") db.ref(`tournaments/${activeTrnId}/standings/${wTI}`).transaction(t => { if (t) t.points = (t.points || 0) + 1; return t; }); }
