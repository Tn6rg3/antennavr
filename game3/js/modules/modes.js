/**
 * MODES.JS
 * Special game modes: Conquest (Co-op), Quiz, Battle Royale, Spectator
 */

// --- CONQUISTA (CO-OP) ---
function startCoopSequence() {
    isCoopMode = true; if (typeof showScreen === 'function') showScreen('gameArea'); if (els.coopArea) els.coopArea.style.display = 'flex'; if (els.gameInputArea) els.gameInputArea.style.display = 'flex'; if (els.pingPongSendArea) els.pingPongSendArea.style.display = 'none'; if (els.tableWrapper) els.tableWrapper.style.display = 'none';
    ...
    inputActive = false;
    if (myId === roomHostId) { const iW = generateCoopTripleWords(); db.ref(`rooms/${roomCode}/coop_state`).set({ progress: 10, timeRemaining: 300, status: 'playing', activeWords: iW, freqOwners: { 1: null, 2: null, 3: null } }); if (typeof startCoopHostTimers === 'function') startCoopHostTimers(); }
    if (typeof listenToCoopState === 'function') listenToCoopState(); if (typeof setupCoopFreqButtons === 'function') setupCoopFreqButtons();
}
...
function listenToCoopState() {
    db.ref(`rooms/${roomCode}/coop_state`).on('value', snap => {
        ...
        if (s.progress >= 100 && s.status !== 'won') { if (myId === roomHostId) db.ref(`rooms/${roomCode}/coop_state/status`).set('won'); if (typeof finishCoopGame === 'function') finishCoopGame(true); return; }
        else if (s.timeRemaining <= 0 || s.status === 'lost') { if (typeof finishCoopGame === 'function') finishCoopGame(false); return; }
        ...
        [1, 2, 3].forEach(n => {
            ...
            else { btn.disabled = true; btn.style.opacity = "0.4"; db.ref(`rooms/${roomCode}/players/${oI}/name`).once('value', sn => { oD.textContent = `🔒 ${sn.val() || 'ALTRO'}`; }); oD.style.color = "#ff9800"; }
        });
        if (coopActiveFreqIndex > 0 && o[coopActiveFreqIndex] === myId && s.activeWords && s.activeWords.length === 3) {
            const cFW = s.activeWords[coopActiveFreqIndex - 1]; if (cFW && cFW !== gameWords[0]) { gameWords[0] = cFW; inputActive = true; if (typeof stopAllMorseAudio === 'function') stopAllMorseAudio(); if (typeof playMorseAudio === 'function') playMorseAudio(cFW, currentWpm); if (els.permanentGameInput) { els.permanentGameInput.value = ""; els.permanentGameInput.focus(); } }
        }
    });
}
        ...
        btn.onclick = () => {
            db.ref(`rooms/${roomCode}/coop_state/freqOwners`).transaction(o => { if (!o) o = { 1: null, 2: null, 3: null }; if (o[n] && o[n] !== myId) return undefined; [1, 2, 3].forEach(x => { if (o[x] === myId) o[x] = null; }); o[n] = myId; return o; }, (err, comm, snap) => {
                if (comm) {
                    ...
                    inputActive = true; db.ref(`rooms/${roomCode}/coop_state/activeWords`).once('value', sn => { const w = sn.val(); if (w && w[n - 1] && lO[n] === myId) { gameWords[0] = w[n - 1]; if (typeof stopAllMorseAudio === 'function') stopAllMorseAudio(); if (typeof playMorseAudio === 'function') playMorseAudio(w[n - 1], currentWpm); if (els.permanentGameInput) els.permanentGameInput.focus(); } });
                } else showToast("⚠️ Frequenza occupata da un compagno!");
            });
        };
    });
    if (els.btnCoopReleaseFreq) { els.btnCoopReleaseFreq.onclick = () => { db.ref(`rooms/${roomCode}/coop_state/freqOwners`).transaction(o => { if (!o) return o; [1, 2, 3].forEach(n => { if (o[n] === myId) o[n] = null; }); return o; }, () => { coopActiveFreqIndex = 0; inputActive = false; if (typeof stopAllMorseAudio === 'function') stopAllMorseAudio(); if (els.permanentGameInput) { els.permanentGameInput.placeholder = "Seleziona prima una Frequenza 🟢🟡🔴..."; els.permanentGameInput.value = ""; } if (els.coopActiveFreqLabel) els.coopActiveFreqLabel.textContent = "Canale: Nessuno selezionato"; if (els.btnCoopReleaseFreq) els.btnCoopReleaseFreq.style.display = 'none'; showToast("🔓 Canale rilasciato per i compagni."); }); }; }
}

function finishCoopGame(won) {
    gameRunning = false; if (typeof clearAllTimers === 'function') clearAllTimers(); if (roomCode) db.ref(`rooms/${roomCode}/coop_state`).off();
    if (roomCode) {
    ...
    if (typeof showScreen === 'function') showScreen('leaderboardScreen'); if (els.tableWrapper) els.tableWrapper.style.display = 'block'; if (els.coopArea) els.coopArea.style.display = 'none';
    if (won) { showToast("🏆 VITTORIA DI SQUADRA! Territorio Conquistato!"); if (els.roomWinnerBanner) { els.roomWinnerBanner.textContent = "🏆 MISSIONE COMPIUTA CONTRO IL DISTURBO NEMICO!"; els.roomWinnerBanner.style.color = "#4caf50"; } if (typeof updateActivity === 'function') updateActivity(true); }
    else { showToast("💀 TEMPO SCADUTO! Il disturbo nemico ha vinto."); if (els.roomWinnerBanner) { els.roomWinnerBanner.textContent = "💀 MISSIONE FALLITA: HA VINTO L'AVVERSARIO IRREALE"; els.roomWinnerBanner.style.color = "#d32f2f"; } if (typeof updateActivity === 'function') updateActivity(false); }
}
...
function startQuizSequence() {
    if (typeof showScreen === 'function') showScreen('quizArea'); gameRunning = true; lastLoadedQuizIndex = -1;
    ...
    if (roomCode && !isSinglePlayer) {
        ...
        listeners.quizState = db.ref(`rooms/${roomCode}/quiz_state`).on('value', snap => {
            ...
            if (nI !== lastLoadedQuizIndex) { lastLoadedQuizIndex = nI; quizQuestionIndex = nI; if (typeof loadNextQuizQuestion === 'function') loadNextQuizQuestion(); }
            quizActiveBuzzerId = s.activeBuzzerId || null; if (typeof renderQuizUI === 'function') renderQuizUI(s);
        });
    ...
    } else { randomizedQuizQuestions = fisherYatesShuffle(aQ); quizQuestionIndex = 0; if (typeof loadNextQuizQuestion === 'function') loadNextQuizQuestion(); }
}

function loadNextQuizQuestion() {
    const mQ = Math.min(requestedWordCount, randomizedQuizQuestions.length);
    if (quizQuestionIndex >= mQ || quizQuestionIndex >= randomizedQuizQuestions.length) { if (typeof finishGame === 'function') finishGame(); return; }
    ...
    currentQuizQuestion = prepareShuffledQuestion(rawQ); setTimeout(() => { if (gameRunning) if (typeof playQuizAudioSequence === 'function') playQuizAudioSequence(); }, 300);
}

async function playQuizAudioSequence() {
    if (!gameRunning || !currentQuizQuestion) return;
    if (typeof stopAllMorseAudio === 'function') stopAllMorseAudio(); inputActive = false; if (typeof disableQuizButtons === 'function') disableQuizButtons(true);
    ...
    if (typeof playMorseAudio === 'function') await playMorseAudio(currentQuizQuestion.q, currentWpm); if (!gameRunning) return; await new Promise(r => setTimeout(r, 1500));
    for (let i = 0; i < 4; i++) {
        ...
        if (typeof playMorseAudio === 'function') await playMorseAudio(`${letter} ${currentQuizQuestion.a[i]}`, currentWpm);
    ...
    if (!gameRunning) return; if (els.quizQuestionBox) els.quizQuestionBox.textContent = "SCEGLI LA TUA RISPOSTA!"; if (typeof enableQuizControls === 'function') enableQuizControls(); if (typeof startQuizTimer === 'function') startQuizTimer(20);
}

function enableQuizControls() {
    inputActive = true; if (isSinglePlayer) { if (typeof disableQuizButtons === 'function') disableQuizButtons(false); }
    else { if (els.quizBuzzer) els.quizBuzzer.style.display = 'block'; if (els.quizOptionsContainer) els.quizOptionsContainer.style.opacity = '0.5'; if (typeof disableQuizButtons === 'function') disableQuizButtons(true); }
}
...
function startQuizTimer(seconds) {
    ...
    quizTimerInterval = setInterval(() => { tL -= 100 / (seconds * 10); if (els.quizTimerProgress) els.quizTimerProgress.style.width = Math.max(0, tL) + '%'; if (tL <= 0) { clearInterval(quizTimerInterval); if (inputActive) { showToast("Tempo scaduto!"); if (isSinglePlayer || quizActiveBuzzerId === myId) if (typeof submitQuizAnswer === 'function') submitQuizAnswer(-1); } } }, 100);
}
...
function submitQuizAnswer(index) {
    ...
    if (quizTimerInterval) clearInterval(quizTimerInterval); inputActive = false; if (typeof disableQuizButtons === 'function') disableQuizButtons(true);
    ...
    setTimeout(() => { if (!gameRunning) return; if (roomCode && !isSinglePlayer) { db.ref(`rooms/${roomCode}/quiz_state`).transaction(s => { if (s && s.activeBuzzerId === myId) { s.questionIndex = (s.questionIndex || 0) + 1; s.activeBuzzerId = null; } return s; }); } else if (isSinglePlayer) { quizQuestionIndex++; if (typeof loadNextQuizQuestion === 'function') loadNextQuizQuestion(); } }, 3000);
}

function renderQuizUI(state) {
    if (!els.quizBuzzer || !els.buzzerWinner || !els.quizOptionsContainer) return;
    if (state.activeBuzzerId) { els.quizBuzzer.style.display = 'none'; if (state.activeBuzzerId === myId) { els.buzzerWinner.textContent = "TOCCA A TE!"; els.quizOptionsContainer.style.opacity = '1'; if (typeof disableQuizButtons === 'function') disableQuizButtons(false); } else { els.buzzerWinner.textContent = "L'AVVERSARIO RISPONDE..."; els.quizOptionsContainer.style.opacity = '0.5'; if (typeof disableQuizButtons === 'function') disableQuizButtons(true); } }
    else { els.buzzerWinner.textContent = ""; els.quizBuzzer.style.display = inputActive ? 'block' : 'none'; els.quizOptionsContainer.style.opacity = '0.5'; if (typeof disableQuizButtons === 'function') disableQuizButtons(true); }
}

function initBattleRoyaleScheduler() { if (typeof checkBattleTime === 'function') checkBattleTime(); if (brCheckInterval) clearInterval(brCheckInterval); brCheckInterval = setInterval(() => { if (typeof checkBattleTime === 'function') checkBattleTime(); }, 100000); }
...
function checkBattleTime() {
    ...
    if (isT) {
        ...
        if (els.btnJoinBR) { els.btnJoinBR.onclick = () => { if (typeof toggleBattleRoyaleJoin === 'function') toggleBattleRoyaleJoin(); if (brBannerTimeout) clearTimeout(brBannerTimeout); brBannerTimeout = setTimeout(() => { if (els.brBanner) els.brBanner.style.display = 'none'; brBannerDismissedToday = true; db.ref(`rooms/${brRoomCode}/players`).off('value'); }, 10000); }; }
    ...
    if (cH === BR_H_START && cM === BR_M_START) { db.ref(`rooms/${brRoomCode}/players/${myId}`).once('value', snap => { if (snap.exists() && activeTab !== "br_playing") { activeTab = "br_playing"; lastBRRoundPlayed = -1; if (typeof showScreen === 'function') showScreen('brScreen'); if (typeof listenToBattleRoyaleRoom === 'function') listenToBattleRoyaleRoom(); } }); if (typeof startBattleRoyaleSystem === 'function') startBattleRoyaleSystem(); }
}

function listenToBattleRoyaleRoom() {
    db.ref(`rooms/${brRoomCode}`).on('value', snap => {
        if (!snap.exists()) { if (typeof showScreen === 'function') showScreen('setupScreen'); alert("La Battaglia è stata annullata o è terminata."); return; }
        const rD = snap.val(); if (typeof renderBRPlayers === 'function') renderBRPlayers(rD.players || {});
        if (rD.status === 'cancelled') { if (els.brStatusText) els.brStatusText.textContent = "Annullata: Giocatori insufficienti (<5)."; setTimeout(() => { if (typeof showScreen === 'function') showScreen('setupScreen'); activeTab = "room"; }, 4000); return; }
        if (rD.status === 'playing') { brIsPlaying = true; if (els.brWpmDisplay) els.brWpmDisplay.textContent = rD.wpm + " WPM"; const mD = rD.players[myId]; brAmIAlive = mD && mD.lives > 0; const h = ["💀 ELIMINATO", "❤️", "❤️❤️", "❤️❤️❤️", "❤️❤️❤️❤️", "❤️❤️❤️❤️❤️"]; let sL = mD && mD.lives ? parseInt(mD.lives) : 0; if (sL < 0) sL = 0; if (sL > 5) sL = 5; if (els.brLivesDisplay) els.brLivesDisplay.textContent = brAmIAlive ? h[sL] : "💀 ELIMINATO"; if (rD.roundEndTime && rD.currentWord && rD.round !== lastBRRoundPlayed) { lastBRRoundPlayed = rD.round; if (typeof handleBRRound === 'function') handleBRRound(rD); } }
...
function startBattleRoyaleSystem() { db.ref(`rooms/${brRoomCode}`).once('value', snap => { const rD = snap.val(); if (rD && rD.hostId === myId) { const pC = Object.keys(rD.players || {}).length; if (pC < 5) db.ref(`rooms/${brRoomCode}/status`).set('cancelled'); else { db.ref(`rooms/${brRoomCode}/status`).set('playing'); if (typeof hostNextBRRound === 'function') hostNextBRRound(rD, 25, 1); } } }); }
...
function hostNextBRRound(rData, wpm, roundNum) {
    ...
    u['currentWord'] = word; u['wpm'] = wpm; u['round'] = roundNum; u['roundEndTime'] = eT; db.ref(`rooms/${brRoomCode}`).update(u); setTimeout(() => { if (typeof checkBRRoundResults === 'function') checkBRRoundResults(wpm, roundNum); }, 31000);
}
...
function handleBRRound(rData) {
    ...
    if (brAmIAlive && !rData.players[myId].answered) { if (els.brInputArea) els.brInputArea.style.display = 'flex'; if (els.brInput) { els.brInput.disabled = false; els.brInput.placeholder = "Decodifica e scrivi qui..."; els.brInput.value = ''; els.brInput.focus(); } if (els.brTimerContainer) els.brTimerContainer.style.display = 'block'; if (typeof playMorseAudio === 'function') playMorseAudio(rData.currentWord, rData.wpm); }
    ...
    brTimerInterval = setInterval(() => {
        ...
        if (l <= 0) { clearInterval(brTimerInterval); if (els.brTimerProgress) els.brTimerProgress.style.width = '0%'; if (brAmIAlive && !rData.players[myId].answered) if (typeof submitBRAnswer === 'function') submitBRAnswer(rData.currentWord, true); }
...
function submitBRAnswer(realWord, isTimeout) {
    if (!brAmIAlive || !els.brInput) return; if (brTimerInterval) clearInterval(brTimerInterval); const typed = els.brInput.value.trim().toUpperCase().substring(0, 50); els.brInput.placeholder = isTimeout ? "Tempo scaduto!" : "Risposta inviata! Attendi..."; els.brInput.value = ''; els.brInput.focus(); const isC = !isTimeout && (typed === realWord);
...
function checkBRRoundResults(currentWpm, currentRound) {
    db.ref(`rooms/${brRoomCode}`).once('value', snap => {
        ...
        else if (typeof hostNextBRRound === 'function') hostNextBRRound(rD, currentWpm + 1, currentRound + 1);
    });
}
...
window.watchSpecificRoom = function(code, targetName) {
    roomCode = code; if (typeof showScreen === 'function') showScreen('gameArea'); if (els.permanentGameInput) { els.permanentGameInput.disabled = true; els.permanentGameInput.placeholder = `👁️ Stai osservando la partita di ${targetName}...`; els.permanentGameInput.value = ""; }
    if (els.wpmDisplay) els.wpmDisplay.textContent = "👁️ SPETTATORE | WPM: --"; if (els.spectatorsCountDisplay) els.spectatorsCountDisplay.style.display = 'none';
    const mSR = db.ref(`rooms/${roomCode}/spectators/${myId}`); mSR.set({ name: myName, ts: firebase.database.ServerValue.TIMESTAMP }); mSR.onDisconnect().remove();
    const rR = db.ref(`rooms/${roomCode}`);
    const oRC = rR.on('value', snap => {
        if (!snap.exists()) { showToast("⚠️ Il giocatore ha terminato o abbandonato la partita."); if (typeof stopWatchingCleanly === 'function') stopWatchingCleanly(); return; }
        const rD = snap.val(); const p = rD.players || {}; const hD = Object.values(p)[0];
        if (!hD || hD.finished) { showToast("🏁 La partita che stavi osservando è terminata!"); if (typeof stopWatchingCleanly === 'function') stopWatchingCleanly(); return; }
        const cS = hD.wpm || rD.wpm || 20; if (els.wpmDisplay) els.wpmDisplay.textContent = `👁️ SPETTATORE | WPM: ${cS}`; if (els.scoreDisplay) els.scoreDisplay.textContent = `Punti: ${hD.score || 0}`;
        if (els.tableBody && hD.matchDetails) {
            els.tableBody.innerHTML = "";
            hD.matchDetails.forEach(row => { const tr = document.createElement('tr'); const tdT = document.createElement('td'); tdT.textContent = row.typed || "-"; const tdR = document.createElement('td'); const bR = document.createElement('b'); if (typeof renderDiffSecure === 'function') renderDiffSecure(bR, row.real, row.typed || ""); tdR.appendChild(bR); const tdP = document.createElement('td'); tdP.style.color = row.points > 0 ? "#4caf50" : "#d32f2f"; tdP.style.fontWeight = "bold"; tdP.textContent = row.points; tr.appendChild(tdT); tr.appendChild(tdR); tr.appendChild(tdP); els.tableBody.appendChild(tr); });
            setTimeout(() => { if (els.tableWrapper) els.tableWrapper.scrollTop = els.tableWrapper.scrollHeight; }, 50);
        }
    });
    const oAC = db.ref(`rooms/${roomCode}/liveAudio`).on('value', snap => { const aD = snap.val(); if (aD?.word) { const lW = aD.wpm || 20; if (els.wpmDisplay) els.wpmDisplay.textContent = `👁️ SPETTATORE | WPM: ${lW}`; if (typeof playMorseAudio === 'function') playMorseAudio(aD.word, lW, true); } });
    window.currentSpectatorCleanup = function() { rR.off('value', oRC); db.ref(`rooms/${roomCode}/liveAudio`).off('value', oAC); mSR.remove(); };
};

function stopWatchingCleanly() { if (typeof window.currentSpectatorCleanup === 'function') { window.currentSpectatorCleanup(); window.currentSpectatorCleanup = null; } setTimeout(() => { roomCode = ""; if (typeof goBackToMenu === 'function') goBackToMenu(); }, 2500); }
