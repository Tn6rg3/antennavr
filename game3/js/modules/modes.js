...
window.watchSpecificRoom = function(code, targetName) {
    roomCode = code; showScreen('gameArea'); if (els.permanentGameInput) { els.permanentGameInput.disabled = true; els.permanentGameInput.placeholder = `👁️ Stai osservando la partita di ${targetName}...`; els.permanentGameInput.value = ""; }
    ...
    const oRC = rR.on('value', snap => {
        if (!snap.exists()) { showToast("⚠️ Il giocatore ha terminato o abbandonato la partita."); stopWatchingCleanly(); return; }
        const rD = snap.val(); const p = rD.players || {}; const hD = Object.values(p)[0];
        if (!hD || hD.finished) { showToast("🏁 La partita che stavi osservando è terminata!"); stopWatchingCleanly(); return; }
        ...
        if (els.tableBody && hD.matchDetails) {
            ...
            hD.matchDetails.forEach(row => { const tr = document.createElement('tr'); const tdT = document.createElement('td'); tdT.textContent = row.typed || "-"; const tdR = document.createElement('td'); const bR = document.createElement('b'); renderDiffSecure(bR, row.real, row.typed || ""); tdR.appendChild(bR); const tdP = document.createElement('td'); tdP.style.color = row.points > 0 ? "#4caf50" : "#d32f2f"; tdP.style.fontWeight = "bold"; tdP.textContent = row.points; tr.appendChild(tdT); tr.appendChild(tdR); tr.appendChild(tdP); els.tableBody.appendChild(tr); });
            setTimeout(() => { if (els.tableWrapper) els.tableWrapper.scrollTop = els.tableWrapper.scrollHeight; }, 50);
        }
    });
    const oAC = db.ref(`rooms/${roomCode}/liveAudio`).on('value', snap => { const aD = snap.val(); if (aD?.word) { const lW = aD.wpm || 20; if (els.wpmDisplay) els.wpmDisplay.textContent = `👁️ SPETTATORE | WPM: ${lW}`; playMorseAudio(aD.word, lW, true); } });
