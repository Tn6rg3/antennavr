...
function getGameWords(num, mode) {
    ...
    if (typeof fisherYatesShuffle === 'function') return fisherYatesShuffle(masterDictionary).slice(0, num).map(w => w.toUpperCase());
    return masterDictionary.slice(0, num).map(w => w.toUpperCase());
}
...
function handleWordSubmission(userWord) {
    ...
    if (currentMode === 'conquest') {
        ...
        if (isCorrect) {
            ...
            showToast(`✅ CORRETTO! +${gain}% (Velocità -> ${currentWpm} WPM)`); if (typeof playBeep === 'function') playBeep(880, 0.1);
            db.ref(`rooms/${roomCode}/coop_state`).transaction(state => {
                ...
                if (!Array.isArray(state.activeWords) || state.activeWords.length !== 3) { if (typeof generateCoopTripleWords === 'function') state.activeWords = generateCoopTripleWords(); return state; }
                const idx = coopActiveFreqIndex - 1; if (idx >= 0 && idx < 3) { if (typeof generateCoopTripleWords === 'function') { const nW = generateCoopTripleWords(); state.activeWords[idx] = nW[idx]; } } return state;
            });
        } else {
            currentWpm = Math.max(10, currentWpm - 2); if (els.wpmDisplay) els.wpmDisplay.textContent = `WPM: ${currentWpm}`;
            showToast(`❌ ERRORE! -${penalty}% (Velocità -> ${currentWpm} WPM)`); if (typeof playBeep === 'function') playBeep(300, 0.25);
        ...
        setTimeout(() => { if (!gameRunning) return; if (els.permanentGameInput) { els.permanentGameInput.value = ""; els.permanentGameInput.focus(); } inputActive = true; if (!isCorrect && gameWords[0]) { if (typeof stopAllMorseAudio === 'function') stopAllMorseAudio(); if (typeof playMorseAudio === 'function') playMorseAudio(gameWords[0], currentWpm); } }, 1500);
        return;
    }
    ...
    else { wordIndex++; setTimeout(() => { if (typeof playNextWord === 'function') playNextWord(); }, 600); }
}

function playNextWord() {
    if (!gameRunning || currentMode === 'pingpong') return; if (wordIndex >= requestedWordCount) { if (typeof finishGame === 'function') finishGame(); return; }
    ...
    if (typeof playMorseAudio === 'function') playMorseAudio(cW, currentWpm); lastWordStartTime = Date.now(); if (els.permanentGameInput) els.permanentGameInput.focus();
}
...
function startCountdownSequence() {
        ...
        db.ref(`rooms/${roomCode}/players`).once('value', snap => {
            ...
            listeners.players = db.ref(`rooms/${roomCode}/players`).on('value', pS => {
                ...
                if (gameStartPlayerCount > 0 && cPC < gameStartPlayerCount) {
                    setTimeout(() => { db.ref(`rooms/${roomCode}/players`).once('value', s => { if (gameRunning && Object.keys(s.val() || {}).length < gameStartPlayerCount) { alert("Un giocatore ha abbandonato. Ritorno al menu."); gameRunning = false; if (typeof exitRoomCleanly === 'function') exitRoomCleanly(false); } else if (gameRunning) showToast("👥 Giocatore rientrato!"); }); }, 10000);
                }
            });
        });
    }
    ...
    if (!isRejoining) { totalScore = 0; wordIndex = 0; quizQuestionIndex = 0; usedReplay = false; sessionCharErrors = Object.create(null); sessionErrorsByWpm = Object.create(null); matchDetailsArray = []; }
    ...
    if (typeof showScreen === 'function') showScreen('countdownScreen'); gameRunning = true; let count = 3; if (els.countdownNumber) els.countdownNumber.textContent = count;
    const interval = setInterval(() => {
        if (count > 1) { count--; if (els.countdownNumber) els.countdownNumber.textContent = count; if (typeof playBeep === 'function') playBeep(600, 0.1); }
        else {
            ...
            if (els.countdownNumber) els.countdownNumber.textContent = (currentLang === 'en' ? 'GO!' : 'VIA!'); if (typeof playBeep === 'function') playBeep(800, 0.3);
            setTimeout(() => {
                ...
                if (currentMode === 'conquest') { if (typeof startCoopSequence === 'function') startCoopSequence(); }
                else if (currentMode === 'quiz') { if (typeof startQuizSequence === 'function') startQuizSequence(); }
                else { if (typeof showScreen === 'function') showScreen('gameArea'); if (currentMode === 'pingpong') { if (typeof setupPingPongListener === 'function') setupPingPongListener(); } else { setTimeout(() => { if (els.permanentGameInput) els.permanentGameInput.focus(); }, 200); setTimeout(() => { if (gameRunning) if (typeof playNextWord === 'function') playNextWord(); }, 800); } }
            }, 500);
        }
    }, 1000);
}
...
function resumeGameSequence() {
    ...
    if (currentMode === 'conquest') { if (typeof startCoopSequence === 'function') startCoopSequence(); } else if (currentMode === 'quiz') { if (typeof startQuizSequence === 'function') startQuizSequence(); } else { if (typeof showScreen === 'function') showScreen('gameArea'); if (currentMode === 'pingpong') { if (typeof setupPingPongListener === 'function') setupPingPongListener(); } else { setTimeout(() => { if (els.permanentGameInput) els.permanentGameInput.focus(); }, 200); setTimeout(() => { if (gameRunning) if (typeof playNextWord === 'function') playNextWord(); }, 800); } }
}
...
function setupPingPongListener() {
    ...
    listeners.pingPong = db.ref(`rooms/${roomCode}/pingpong`).on('value', snap => {
        ...
        if (ppD.lastGuess && ppD.lastGuess.id !== window.lastSeenGuessId) {
            window.lastSeenGuessId = ppD.lastGuess.id; const tr = document.createElement('tr'); const tdT = document.createElement('td'); tdT.textContent = ppD.lastGuess.typed || ''; const tdR = document.createElement('td'); if (typeof renderDiffSecure === 'function') renderDiffSecure(tdR, ppD.lastGuess.real, ppD.lastGuess.typed || '');
        ...
        if (ppD.wordsPlayed >= requestedWordCount) { if (ppTimerInterval) clearInterval(ppTimerInterval); if (typeof finishGame === 'function') finishGame(); return; }
        if (ppD.senderId === myId) {
            if (!ppD.word) { if (els.pingPongSendArea) els.pingPongSendArea.style.display = 'flex'; if (els.gameInputArea) els.gameInputArea.style.display = 'none'; if (els.pingPongWordToSend) { els.pingPongWordToSend.value = ''; setTimeout(() => els.pingPongWordToSend.focus(), 100); } if (typeof startPingPongTimer === 'function') startPingPongTimer(); }
...
function startPingPongTimer() {
    ...
    ppTimerInterval = setInterval(() => { tL -= (100 / 300); if (els.pingPongTimerProgress) els.pingPongTimerProgress.style.width = Math.max(0, tL) + '%'; if (tL <= 0) { clearInterval(ppTimerInterval); if (typeof sendAutoPingPongWord === 'function') sendAutoPingPongWord(); } }, 100);
}
...
function finishGame() {
    gameRunning = false; inputActive = false; if (els.permanentGameInput) els.permanentGameInput.blur(); if (typeof clearAllTimers === 'function') clearAllTimers();
    if (listeners.pingPong) { db.ref(`rooms/${roomCode}/pingpong`).off('value', listeners.pingPong); listeners.pingPong = null; }
    ...
    if (typeof showScreen === 'function') showScreen('leaderboardScreen');
    ...
    if (currentMode === 'daily_challenge') { let tS = new Date().toISOString().split('T')[0]; localStorage.setItem(STORAGE_DAILY_SHOWN, tS); activeTab = "daily_challenge"; if (typeof showLeaderboardTab === 'function') showLeaderboardTab('opt_lb_daily'); }
