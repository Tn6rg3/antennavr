...
function joinRoomLogic(isReconnect = false) {
    ...
    pRef.once('value', snapshot => {
        ...
        listeners.room.on('value', snap => {
            ...
            if (rD.status === 'playing' || rD.status === 'countdown' || rD.status === 'waiting') localStorage.setItem(STORAGE_ROOM_KEY, roomCode);
...
function exitRoomCleanly(roomWasDeletedByHost = false, isExplicitQuit = false) {
    ...
    if (roomCode) {
        if (roomCode.startsWith("TRN_")) targetS = 'teamsScreen';
        if (roomWasDeletedByHost) {
            localStorage.removeItem(STORAGE_ROOM_KEY);
            if (amIH && !roomCode.startsWith("TRN_")) { db.ref(`rooms/${roomCode}`).remove(); db.ref(`public_lobby_rooms/${roomCode}`).remove(); }
            else { db.ref(`rooms/${roomCode}/players/${myId}`).onDisconnect().cancel(); db.ref(`rooms/${roomCode}/players/${myId}`).remove(); }
            roomCode = "";
            ...
            // Se non è eliminazione stanza o abbandono esplicito (es: clicco "Indietro"), non rimuoviamo STORAGE_ROOM_KEY
