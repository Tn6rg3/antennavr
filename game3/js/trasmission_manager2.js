...
window.initTransmissionManager = function() {
    ...
    if (!window.transmissionGlobalListenersReadyV2) {
        ...
        // Evita menu contestuale quando il destro è mappato o durante mapping
        window.addEventListener('contextmenu', (e) => {
            const mappingActive = !!window.keyerState.mappingTarget;
            const rightUsed =
                window.keyerState.keyDit === 'MOUSE_RIGHT' ||
                window.keyerState.keyDah === 'MOUSE_RIGHT' ||
                window.keyerState.keyVert === 'MOUSE_RIGHT';

            if (mappingActive || (window.keyerState.enabled && rightUsed)) {
                e.preventDefault();
            }
        });
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (window.keyerState.mode === 'V') {
                if (window.keyMatchesInput(window.keyerState.keyVert, e)) {
                    e.preventDefault();
            ...
            if (window.keyMatchesInput(window.keyerState.keyDit, e)) {
                e.preventDefault();
                if (!window.keyerState.isDitDown) {
                    window.keyerState.isDitDown = true;
                    window.processKeyerInput();
                }
            } else if (window.keyMatchesInput(window.keyerState.keyDah, e)) {
                e.preventDefault();
        ...
        window.addEventListener('keyup', (e) => {
            ...
            if (window.keyMatchesInput(window.keyerState.keyDit, e)) window.keyerState.isDitDown = false;
            if (window.keyMatchesInput(window.keyerState.keyDah, e)) window.keyerState.isDahDown = false;
        });

        // MOUSE DOWN (mapping + uso keyer)
        window.addEventListener('mousedown', (e) => {
            ...
            // Mapping da mouse
            if (window.keyerState.mappingTarget) {
                const mapped = (e.button === 0) ? 'MOUSE_LEFT' : (e.button === 2 ? 'MOUSE_RIGHT' : null);
                if (mapped) {
                    e.preventDefault();
                    window.mapInputToTarget(mapped);
                }
                return;
            }
...
window.loadKeyerSettings = function() {
    ...
    if (saved) {
        try {
            ...
            window.keyerState.enabled = (typeof s.enabled === 'boolean') ? s.enabled : false;
...
// ====== NUOVI HELPER INPUT (TASTIERA + MOUSE) ======
window.getInputLabel = function(k) {
    if (k === " ") return "Spazio";
    if (k === "MOUSE_LEFT") return "Mouse Sinistro";
    if (k === "MOUSE_RIGHT") return "Mouse Destro";
    return k || "Nessuno";
};

window.mapInputToTarget = function(inputValue) {
    if (!window.keyerState.mappingTarget) return false;

    if (window.keyerState.mappingTarget === 'dit') window.keyerState.keyDit = inputValue;
    else if (window.keyerState.mappingTarget === 'dah') window.keyerState.keyDah = inputValue;
    else if (window.keyerState.mappingTarget === 'vert') window.keyerState.keyVert = inputValue;

    window.keyerState.mappingTarget = null;
    window.updateKeyerUI();
    window.saveKeyerSettings();
    showToast("Tasto assegnato: " + window.getInputLabel(inputValue));
    return true;
};

window.keyMatchesInput = function(savedKey, e) {
    // Tastiera
    if (typeof e.key === 'string' && e.key === savedKey) return true;

    // Mouse
    if (savedKey === 'MOUSE_LEFT' && typeof e.button === 'number' && e.button === 0) return true;
    if (savedKey === 'MOUSE_RIGHT' && typeof e.button === 'number' && e.button === 2) return true;
...
window.updateKeyerUI = function() {
    ...
    if (btnDit) {
        btnDit.textContent = "Tasto: " + window.getInputLabel(window.keyerState.keyDit);
        btnDit.classList.remove('pulse');
    }
    if (btnDah) {
        btnDah.textContent = "Tasto: " + window.getInputLabel(window.keyerState.keyDah);
        btnDah.classList.remove('pulse');
    }
    if (btnVert) {
        btnVert.textContent = "Tasto: " + window.getInputLabel(window.keyerState.keyVert);
