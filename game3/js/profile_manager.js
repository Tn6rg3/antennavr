...
if (els.deleteDataBtn) {
    els.deleteDataBtn.onclick = async () => {
        if (!confirm("ATTENZIONE: Questa azione eliminerà DEFINITIVAMENTE tutto il tuo profilo ed i tuoi dati dal gioco.\n\nVuoi procedere?")) return;
        if (!confirm("CONFERMA FINALE: Sei assolutamente sicuro? Verranno rimossi i tuoi record, la tua presenza ed il tuo profilo.")) return;
        ...
        try {
            ...
            const dKey = now.toISOString().split('T')[0];
            const getWeekKey = (d) => {
                if (typeof window.getWeekNumber === 'function') return window.getWeekNumber(d);
            ...
            // FASE 2: Pulizia Richieste Tutor e Attivita
            try {
            ...
            // FASE 4: Rimozione Dati Utente Principali (Mentre uid_mapping e ancora attivo!)
            await Promise.all([
            ...
            // FASE 5: Rimozione uid_mapping SOLTANTO COME ULTIMISSIMO PASSAGGIO!
            if (firebaseUid) {
                await safeRemove(db.ref(`uid_mapping/${firebaseUid}`));
            }

            showToast("Profilo eliminato. Chiusura in corso...");

            // FASE 6: Pulizia completa cache e Disconnessione
            localStorage.clear();
            ...
            // FASE 7: Uscita immediata dal gioco (Chiusura WebApp Telegram)
            setTimeout(() => {
