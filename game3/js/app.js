    ...
    try {
        ...
        els.regolamentoContainer.innerHTML = `
            <div style="text-align:center; padding: 15px;">
                ...
                <p style="font-size:0.75em; color:var(--hint-color);"><i>Nota: Caricamento regolamento fallito (${e.message}).</i></p>
            </div>`;
    }
};
...
function checkGameTypeUI() {
    ...
    if (isCoop) {
        if (els.createRoomBtn) els.createRoomBtn.textContent = currentLang === 'it' ? "Crea Stanza Co-op ⚔️" : "Create Co-op Room ⚔️";
    } else if (isTrn) {
        if (els.createRoomBtn) els.createRoomBtn.textContent = currentLang === 'it' ? "Vai all'Area Tornei" : "Go to Tournaments";
    } else {
...
function initGame() {
    ...
    auth.signInAnonymously().then(async () => {
        ...
        db.ref('.info/connected').on('value', (snap) => {
            ...
            if (roomCode) {
                if (typeof window.joinRoomLogic === 'function') window.joinRoomLogic(true);
            }
        });
        ...
        if (startParam) {
            if (startParam.startsWith('team_')) {
                if (typeof window.processTeamInvite === 'function') window.processTeamInvite(startParam.replace('team_', ''));
            } else if (startParam.startsWith('room_')) {
                if (typeof window.joinSpecificRoom === 'function') window.joinSpecificRoom(startParam.replace('room_', ''));
            }
        } else {
            ...
            if (lastRoom) {
                db.ref(`rooms/${lastRoom}`).once('value', snap => {
                    if (snap.exists() && snap.val().status !== 'finished') {
                        ...
                        if (els.rejoinGameBtn) els.rejoinGameBtn.onclick = () => { isRejoining = true; if (typeof window.joinRoomLogic === 'function') window.joinRoomLogic(false); };
        ...
        if (savedCustom) { 
            try { 
                window.customDictionary = JSON.parse(savedCustom);
                if (typeof updateCustomDictStatus === 'function') updateCustomDictStatus();
            } catch(e) {} 
        }
    ...
    // Forza refresh UI dopo caricamento config se window.GAME_MODES esiste già
    setTimeout(() => { if (typeof checkGameTypeUI === 'function') checkGameTypeUI(); }, 500);
}
// ============================================================================
