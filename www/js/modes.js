...
function startCoopSequence() {
    ...
    if (els.permanentGameInput) {
        ...
        els.permanentGameInput.placeholder = "Seleziona prima una Frequenza ðŸŸ¢ðŸŸ¡ðŸ”´...";
...
function listenToCoopState() {
    db.ref(`rooms/${roomCode}/coop_state`).on('value', snap => {
        ...
        if (els.coopTimeDisplay) els.coopTimeDisplay.textContent = `â±ï¸ ${mins}:${secs}`;
...
function setupCoopFreqButtons() {
    const labels = ["ðŸŸ¢ FREQ 1 (3-4 car.)", "ðŸŸ¡ FREQ 2 (5-6 car.)", "ðŸ”´ FREQ 3 (7+ car.)"];
        ...
        btn.onclick = () => {
            db.ref(`rooms/${roomCode}/coop_state/freqOwners`).transaction(owners => {
                ...
                if (committed) {
                    ...
                    showToast("âš ï¸ Frequenza occupata da un compagno!");
                }
            });
        };
    });
    ...
    if (els.btnCoopReleaseFreq) {
        els.btnCoopReleaseFreq.onclick = () => {
            db.ref(`rooms/${roomCode}/coop_state/freqOwners`).transaction(owners => {
                ...
                if (els.coopActiveFreqLabel) els.coopActiveFreqLabel.textContent = "Canale: Nessuno selezionato";
                if (els.btnCoopReleaseFreq) els.btnCoopReleaseFreq.style.display = 'none';
                showToast("ðŸ”“ Canale rilasciato per i compagni.");
            });
        };
    }
}
...
function finishCoopGame(won) {
    ...
    if (won) {
        showToast("ðŸ† VITTORIA DI SQUADRA! Territorio Conquistato!");
        if (els.roomWinnerBanner) {
            els.roomWinnerBanner.textContent = "ðŸ† MISSIONE COMPIUTA CONTRO IL DISTURBO NEMICO!";
            els.roomWinnerBanner.style.color = "#4caf50";
        }
        updateActivity(true);
    } else {
        showToast("ðŸ’€ TEMPO SCADUTO! Il disturbo nemico ha vinto.");
        if (els.roomWinnerBanner) {
            els.roomWinnerBanner.textContent = "ðŸ’€ MISSIONE FALLITA: HA VINTO L'AVVERSARIO IRREALE";
...
// Domande di emergenza se quiz_data.js non Ã¨ raggiungibile
const FALLBACK_QUIZ_QUESTIONS = [
    ...
    { q: "QTH", a: ["La mia posizione Ã¨...", "Qual Ã¨ il tuo nome?", "Chiudi la trasmissione", "Segnale disturbato"], correct: 0 },
    { q: "QRS", a: ["Trasmetti piÃ¹ lentamente", "Aumenta velocitÃ ", "Frequenza occupata", "Ripeti messaggio"], correct: 0 },
    ...
    db.ref(`rooms/${brRoomCode}/players/${myId}`).once('value', pSnap => {
        if (pSnap.exists()) {
            ...
            db.ref(`rooms/${brRoomCode}/players/${myId}`).set({
                ...
                showToast("âš”ï¸ Iscrizione registrata! Il banner Ã¨ ora verde.");
            });
        }
    });
};
...
function listenToBattleRoyaleRoom() {
    db.ref(`rooms/${brRoomCode}`).on('value', snap => {
        if (!snap.exists()) {
            ...
            alert("La Battaglia Ã¨ stata annullata o Ã¨ terminata.");
            return;
        }
        ...
        if (rData.status === 'playing') {
            ...
            const hearts = ["ðŸ’€ ELIMINATO", "â¤ï¸", "â¤ï¸â¤ï¸", "â¤ï¸â¤ï¸â¤ï¸", "â¤ï¸â¤ï¸â¤ï¸â¤ï¸", "â¤ï¸â¤ï¸â¤ï¸â¤ï¸â¤ï¸"];
            let safeLives = myData && myData.lives ? parseInt(myData.lives) : 0;
            if (safeLives < 0) safeLives = 0;
            if (safeLives > 5) safeLives = 5;
            if (els.brLivesDisplay) els.brLivesDisplay.textContent = brAmIAlive ? hearts[safeLives] : "ðŸ’€ ELIMINATO";
...
function renderBRPlayers(players) {
    ...
    Object.values(players).forEach(p => {
        ...
        const heartsList = ["ðŸ’€", "â¤ï¸", "â¤ï¸â¤ï¸", "â¤ï¸â¤ï¸â¤ï¸", "â¤ï¸â¤ï¸â¤ï¸â¤ï¸", "â¤ï¸â¤ï¸â¤ï¸â¤ï¸â¤ï¸"];
...
window.watchSpecificRoom = function(code, targetName) {
    ...
    if (els.permanentGameInput) {
        ...
        els.permanentGameInput.placeholder = `ðŸ‘ï¸ Stai osservando la partita di ${targetName}...`;
        els.permanentGameInput.value = "";
    }

    if (els.wpmDisplay) els.wpmDisplay.textContent = "ðŸ‘ï¸ SPETTATORE | WPM: --";
    ...
    const onRoomChange = roomRef.on('value', snap => {
        if (!snap.exists()) {
            showToast("âš ï¸ Il giocatore ha terminato o abbandonato la partita.");
            stopWatchingCleanly();
            return;
        }
        ...
        if (!hostData || hostData.finished) {
            showToast("ðŸ La partita che stavi osservando Ã¨ terminata!");
            stopWatchingCleanly();
            return;
        }

        const currentSpeed = hostData.wpm || roomData.wpm || 20;
        if (els.wpmDisplay) els.wpmDisplay.textContent = `ðŸ‘ï¸ SPETTATORE | WPM: ${currentSpeed}`;
    ...
    const onAudioChange = db.ref(`rooms/${roomCode}/liveAudio`).on('value', snap => {
        ...
        if (audioData && audioData.word) {
            ...
            if (els.wpmDisplay) els.wpmDisplay.textContent = `ðŸ‘ï¸ SPETTATORE | WPM: ${liveWpm}`;
