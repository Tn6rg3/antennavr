// --- ATTIVITÃ€, MEDAGLIE E STATISTICHE UTENTE ---
...
async function checkActivityAndAwardMedals() {
    ...
    try {
        ...
        check(dData.games, 3, 'd_bronze', "Bronzo Giornaliero", "Hai giocato 3 partite oggi!", "ðŸ¥‰", dKey);
        check(dData.games, 7, 'd_silver', "Argento Giornaliero", "Sei un veterano! 7 partite oggi!", "ðŸ¥ˆ", dKey);
        check(dData.games, 15, 'd_gold', "Oro Giornaliero", "Incredibile! 15 partite in un giorno!", "ðŸ¥‡", dKey);
        check(wData.games, 20, 'w_active', "Stakanovista Settimanale", "20 partite questa settimana!", "ðŸŽ–ï¸", wKey);
        check(wData.games, 50, 'w_pro', "Campione Settimanale", "50 partite! Una leggenda questa settimana!", "ðŸ†", wKey);
        check(mData.games, 150, 'm_legend', "Titano del Mese", "150 partite! Il gioco non ha segreti per te.", "ðŸ’Ž", mKey);
    } catch(e) {}
    updateMedalGallery();
}
...
window.switchActTab = function(period) {
    ...
    if (els.actListTitle) {
        els.actListTitle.textContent = period === 'daily' ? "I piÃ¹ attivi di Oggi" : period === 'weekly' ? "I piÃ¹ attivi della Settimana" : "I piÃ¹ attivi del Mese";
    }
...
function renderActivityRankings(period, key) {
    ...
    db.ref(`activity/${period}/${key}`).once('value').then(snap => {
        ...
        if (users.length === 0) {
            const empLi = document.createElement('li'); empLi.style.cssText = "justify-content:center; color:var(--hint-color);"; empLi.textContent = "Nessuna attivitÃ  registrata."; els.activityRankList.appendChild(empLi); return;
        }
        users.forEach((u, idx) => {
...
if (els.deleteDataBtn) {
    els.deleteDataBtn.addEventListener('click', async () => {
        if (confirm("âš ï¸ Eliminerai per sempre TUTTI i tuoi dati. Confermi?")) {
            try {
...
if (els.saveAliasBtn) {
    els.saveAliasBtn.addEventListener('click', async () => {
        ...
        if (privacy && !alias) return alert("L'Alias Ã¨ obbligatorio se nascondi lo username Telegram!");
...
if (els.resetStatsBtn) {
    els.resetStatsBtn.addEventListener('click', async () => {
        if (confirm(currentLang === 'it' ? "Vuoi azzerare tutte le tue statistiche? Questa operazione non puÃ² essere annullata." : "Reset all your statistics? This cannot be undone.")) {
            try {
...
window.showProfileScreen = function() {
    ...
    db.ref(`users/${myId}/history`).orderByChild('date').limitToLast(30).once('value').then(snap => {
        ...
        userMatchHistory.forEach(match => {
            ...
            let modeIcon = match.mode === 'callsign' ? 'ðŸŽ™ï¸ Nom.' : match.mode === 'pingpong' ? 'ðŸ“ Ping Pong' : match.mode === 'chars' ? 'âŒ¨ï¸ Carat.' : (match.mode === 'daily_challenge' ? 'ðŸ“… Daily' : match.mode === 'quiz' ? 'â“ Quiz' : 'ðŸ”¤ Parole');
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        // 1. Lo schermo si Ã¨ spento o l'utente ha ridotto Telegram a icona
        ...
        if (gameRunning && window.lostFocusDuringWord) {
            window.lostFocusDuringWord = false;
            inputActive = false;
            showToast("âš ï¸ Schermo spento: parola considerata persa!");

            // Gestione specifica in base alla modalitÃ  di gioco
            if (currentMode === 'conquest') {
                // In Conquista (Co-op) applichiamo una piccola penalitÃ  e rigeneriamo la parola
                ...
                // Non resettiamo currentWpm a baseWpm! Applichiamo solo la normale penalitÃ  (-2 WPM)
