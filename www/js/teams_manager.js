...
function listenToMyTeam() {
    ...
    listeners.team = db.ref(`teams/${myTeamId}`).on('value', snap => {
        ...
        if (els.teamStatusText) els.teamStatusText.innerHTML = team.status === 'open' ? 'ðŸŸ¢ Adesioni Aperte' : 'ðŸ”´ Adesioni Chiuse';
...
function renderActiveTournament(trnSnap) {
    ...
    if (els.trnStandingsBody) {
        ...
        std.forEach((s, idx) => {
            let med = idx === 0 ? 'ðŸ¥‡' : idx === 1 ? 'ðŸ¥ˆ' : idx === 2 ? 'ðŸ¥‰' : `${idx + 1}.`;
    ...
    if (els.trnBracketContainer) {
        ...
        if (trn.status === 'open') {
            ...
            waitP.textContent = currentLang === 'it' ? "Il torneo Ã¨ aperto, attendi l'avvio dall'organizzatore." : "The tournament is open, wait for the host to start.";
            ...
            Object.entries(trn.matches).forEach(([mId, m]) => {
                ...
                if (m.status !== 'finished') {
                    ...
                    btnA.innerHTML = m.playerA
                        ? `âœ… <b>${escapeHTML(m.playerA.name)}</b><br><small>(${escapeHTML(m.teamAName)})</small>`
                        : `ðŸŸ¢ <b>Scegli per ${escapeHTML(m.teamAName)}</b><br><small>(Posto A)</small>`;
                    ...
                    btnB.innerHTML = m.playerB
                        ? `âœ… <b>${escapeHTML(m.playerB.name)}</b><br><small>(${escapeHTML(m.teamBName)})</small>`
                        : `ðŸŸ¢ <b>Scegli per ${escapeHTML(m.teamBName)}</b><br><small>(Posto B)</small>`;
                    ...
                    if (m.playerA && m.playerB && (m.playerA.id === myId || m.playerB.id === myId)) {
                        ...
                        joinBtn.textContent = currentLang === 'it' ? 'âš¡ ENTRA NELLA SFIDA' : 'âš¡ JOIN MATCH';
...
window.toggleTrnSlot = function(matchId, side, teamId, targetTeamName = "questa squadra") {
    ...
    slotRef.once('value', snap => {
        if (!snap.exists()) {
            ...
            alert("âš ï¸ Questo posto Ã¨ giÃ  stato occupato da " + snap.val().name);
        }
    });
};
