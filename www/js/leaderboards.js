...
function renderRoomLeaderboard(players) {
    ...
    if (allFinished && playersArray.length > 0 && els.roomWinnerBanner) {
        els.roomWinnerBanner.textContent = roomCode.startsWith("TRN_") ? `ðŸ† Vince il match: ${playersArray[0].name}` : `ðŸ† Vincitore: ${playersArray[0].name}`;
    }
}
...
function renderHeadToHeadView(players, container) {
    ...
    players.forEach((p) => {
        ...
        const sSpLbl = document.createElement('span'); sSpLbl.textContent = currentLang === 'it' ? 'VelocitÃ :' : 'Speed:';
...
function renderMatchesHistoryHTML(matches, container) {
    ...
    matches.forEach(match => {
        ...
        infoDiv.textContent = `ðŸ“… ${match.date} - ${match.wordCount} Stringhe`; mw.appendChild(infoDiv);
...
function renderPlayersListHTML(players, container, showWordCount, isTeam = false) {
    ...
    players.forEach((player, index) => {
        ...
        if (index === 0) medalDiv.textContent = "ðŸ¥‡"; else if (index === 1) medalDiv.textContent = "ðŸ¥ˆ"; else if (index === 2) medalDiv.textContent = "ðŸ¥‰";
